...
`unknown` is the result of `bin/fm-harness.sh`'s own two-tier detection (environment markers, then process ancestry), not an unexamined default.
It is this safety boundary's consequence, not a general ambiguity policy, so ask the captain rather than guessing or dispatching on an unverified adapter.
