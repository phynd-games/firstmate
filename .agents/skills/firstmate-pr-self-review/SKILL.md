...
The surface section must contain `Authority:`, `Security:`, `Path:`, `Failure:`, `Tests:`, `Documentation:`, and `Delivery:` fields in the form `reviewed; surface=<surface>; files=<...>; evidence=<path>:<line> sha256=<line-digest> change-sha256=<change-digest> line-hex=<changed-line-bytes> before-hex=<old-line-bytes|none> after-hex=<new-line-bytes|none> hunk=<path>:<line> claim=observed:<path>:<line>:<line-digest>:<changed-line-bytes> behavior=<surface-behavior> behavior-sha256=<behavior-proof>; consequence=anchor=<path>:<line> side=<old|new> sha256=<line-digest> change-sha256=<change-digest> line-hex=<changed-line-bytes> before-hex=<old-line-bytes|none> after-hex=<new-line-bytes|none> hunk=<path>:<line> claim=transition:<old-line-bytes|none>-><new-line-bytes|none> behavior=<surface-behavior> behavior-sha256=<behavior-proof> binding=<surface-binding>; fix=anchor=<path>:<line> side=<old|new> sha256=<line-digest> change-sha256=<change-digest> line-hex=<changed-line-bytes> before-hex=<old-line-bytes|none> after-hex=<new-line-bytes|none> hunk=<path>:<line> claim=applied:<path>:<line>:<line-digest>:<changed-line-bytes> behavior=<surface-behavior> action=<surface-action> behavior-sha256=<behavior-proof> binding=<surface-binding>`, with each surface `files=` list containing actual changed files and each evidence path belonging to that surface's changed-file owner class, each evidence, consequence, and fix reference naming the same listed tracked file and actual changed hunk, each claim spelling out the independently verified observation, complete before/after transition, or applied line, and each line digest and byte encoding being independently derived from its declared added or deleted line. The behavior proof is the exact lowercase SHA-256 of the surface, all three anchor references and sides, both before/after transition encodings, the changed-hunk digest, the required surface behavior/action, and the three role-specific claims. The binding must cover the behavior proof and all three exact anchors, so a surface claim cannot be detached from the verified transition it describes.
When a surface has no applicable changed path, use `reviewed; surface=<surface>; scope=unaffected; files=<...>; rationale=no applicable changed <surface> surface; binding=<surface-binding>` instead of reusing unrelated evidence.
For that form, bind `<surface>|unaffected|<files>|<changed-files-digest>|<surface-behavior>|<surface-action>` with a lowercase SHA-256 digest, and list the actual changed paths in `files=`.
Applicable surfaces must use distinct changed hunk references, so one changed line cannot stand in for independent review of multiple surfaces.
The surface binding must be the lowercase SHA-256 digest of `<surface>|<evidence-path>:<line>|<evidence-line-digest>|<change-digest>|<evidence-line-bytes>|<consequence-path>:<line>|<consequence-side>|<consequence-line-digest>|<consequence-line-bytes>|<fix-path>:<line>|<fix-side>|<fix-line-digest>|<fix-line-bytes>`, so consequence and fix records are bound to independently verifiable before/after diff evidence rather than reusable surface labels or prose.
