...
The surface section must contain `Authority:`, `Security:`, `Path:`, `Failure:`, `Tests:`, `Documentation:`, and `Delivery:` fields in the form `reviewed; surface=<surface>; files=<...>; evidence=<path>:<line> sha256=<line-digest> hunk=<path>:<line>; consequence=anchor=<path>:<line> sha256=<line-digest> behavior=<surface-behavior> binding=<surface-binding>; fix=anchor=<path>:<line> sha256=<line-digest> action=<surface-action> binding=<surface-binding>`, with each evidence reference naming a listed tracked file from the target diff and the lowercase SHA-256 digest of its referenced line.
The surface binding must be the lowercase SHA-256 digest of `<surface>|<path>:<line>|<line-digest>|<surface-behavior>|<surface-action>`, so consequence and fix records are bound to the declared surface and exact changed hunk rather than reusable prose.
