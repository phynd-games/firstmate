...
The surface section must contain `Authority:`, `Security:`, `Path:`, `Failure:`, `Tests:`, `Documentation:`, and `Delivery:` fields in the form `reviewed; surface=<surface>; files=<...>; evidence=<path>:<line> sha256=<line-digest> change-sha256=<change-digest> line-hex=<changed-line-bytes> hunk=<path>:<line>; consequence=anchor=<path>:<line> sha256=<line-digest> change-sha256=<change-digest> line-hex=<changed-line-bytes> behavior=<surface-behavior> binding=<surface-binding>; fix=anchor=<path>:<line> sha256=<line-digest> change-sha256=<change-digest> line-hex=<changed-line-bytes> action=<surface-action> binding=<surface-binding>`, with each evidence reference naming a listed tracked file from the target diff, the lowercase SHA-256 digest of its referenced added or deleted line, the lowercase SHA-256 digest of the exact no-color diff for that changed path, and the lowercase byte encoding of the exact referenced changed line.
...
The surface binding must be the lowercase SHA-256 digest of `<surface>|<path>:<line>|<line-digest>|<change-digest>|<changed-line-bytes>|<surface-behavior>|<surface-action>`, so consequence and fix records are bound to the declared surface, exact changed hunk, actual changed-path content, and an exact behavior anchor rather than reusable prose.
