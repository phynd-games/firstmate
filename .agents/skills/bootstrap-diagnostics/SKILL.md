...
- `BACKEND_INVALID: <name>|none - REFUSED: ...` - the home's runtime backend declaration is absent, empty, or not `herdr`, and Herdr is the sole supported runtime backend (`AGENTS.md` hard rule 6), so do not dispatch work until the printed remediation is applied: write exactly `herdr` to `config/backend` (or export `FM_BACKEND=herdr`) and confirm the Herdr runtime; never work around it by selecting or installing another backend, and treat a runtime marker named in the line as evidence only.
  Unknown backend names always use the Herdr-only `BACKEND_INVALID` refusal and the same correction.
...
- `LAUNCH_RECONCILE: <kind>:<subject> launch=<id> phase=<phase> ... ` - a launch record is open with an unknown external effect or its launcher died before settling it, so the named worker or helper may or may not exist in Herdr.
  ...
  With a recorded native identity, the next launch through the ordinary owner settles only what native evidence settles: a gone pane reads absent, a live agent refuses the duplicate (stop it with `bin/fm-control.sh <id> exit` or relaunch it), and a present pane with no registered agent refuses with a diagnostic (busy foreground, idle shell, attached child) because none of those excludes a launch still starting or a process that detached from the pane - stop or relaunch it through control so the stop is recorded, tear the task down, or settle the record after inspecting the pane natively.
  Without a recorded identity, the next launch refuses and prints what carries the task's label as a hint: inspect that endpoint natively (`herdr tab list --workspace <ws>`, `herdr agent get <pane>`, `herdr pane process-info --pane <pane>` on the named session), close a leftover through its owner when you have positively established it is this task's dead remains and nothing else, and only then settle the record with the exact `reconcile --verdict manual --evidence "<what you verified>"` command the refusal prints.
  A label match never establishes ownership, and no label match never establishes that nothing was created; `manual` records that a person or Firstmate looked, so write what was actually verified in the evidence.
  Reconcile a helper record by running that helper's own `ensure` or `start`; a `watcher` record is settled by the next arm cycle the emitted supervision protocol runs, and a `procevent-<source>` record by that source's next `bin/fm-procevent.sh start` or `reconcile`.
  `docs/launch-records.md` owns the contract and its limits.
