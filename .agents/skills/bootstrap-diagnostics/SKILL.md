...
- `BACKEND_INVALID: <name>|none - REFUSED: ...` - the home's runtime backend declaration is absent, empty, or not `herdr`, and Herdr is the sole supported runtime backend (`AGENTS.md` hard rule 6), so do not dispatch work until the printed remediation is applied: write exactly `herdr` to `config/backend` (or export `FM_BACKEND=herdr`) and confirm the Herdr runtime; never work around it by selecting or installing another backend, and treat a runtime marker named in the line as evidence only.
  The lane-only `BACKEND_INVALID: <name> (known: <names>)` form appears when an unknown name reaches the retained-adapter regression lane and needs the same correction.
