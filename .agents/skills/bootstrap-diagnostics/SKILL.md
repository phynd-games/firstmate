...
- `LAUNCH_RECONCILE: <kind>:<subject> launch=<id> phase=<phase> ... ` - a launch record is open with an unknown external effect or its launcher died before settling it, so the named worker or helper may or may not exist in Herdr.
  ...
  With a recorded native identity, the next launch through the ordinary owner settles it from native evidence: a gone pane reads absent, a live agent refuses the duplicate (stop it with `bin/fm-control.sh <id> exit` or relaunch it), an agent-free pane with an idle foreground shell is replaced, and an agent-free pane with a busy foreground refuses because a launch may still be starting - wait for it to register or stop it through control.
