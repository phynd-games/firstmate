...
- Run `bin/fm-lint.sh` before treating a script change as done; its header and `--help` are the single owner of the lint definition (file set, config, pinned shellcheck version, and pinned actionlint workflow lint) that the no-mistakes pre-push gate invokes, and it refuses to run under any other version of either linter.
