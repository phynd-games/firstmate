...
6. Run `no-mistakes` to attach to the pipeline, watch findings, authorize auto-fixes, and review ask-user findings as needed.
   ...
   If the pipeline applies a fix or any later commit changes the branch, push the resulting head through `no-mistakes` again so the PR attestation matches the current commit.
