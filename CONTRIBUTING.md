...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass locally and in the no-mistakes pre-push gate; its header and `--help` are the single owner of the lint definition (the shellcheck file set, config, pinned shellcheck version, and pinned actionlint workflow lint) and its full-analysis default; the hosted CI lint job was retired by captain instruction on 2026-09-09.
...
See [Gate defaults](docs/configuration.md#gate-defaults-no-mistakesyaml) for the repository's evidence publication and lint configuration.
Local no-mistakes Test is intent-targeted and must not re-run every `tests/*.test.sh`; [`.github/workflows/ci.yml`](.github/workflows/ci.yml) owns hosted check selection and exclusion rationale.
...
```sh
while IFS= read -r script; do /bin/bash -n "$script" || exit; done < <(bin/fm-lint.sh --list-files)   # syntax-check the context-selected shell surface
bin/fm-lint.sh   # canonical local and pre-push lint entry point
