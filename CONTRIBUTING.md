...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, pinned shellcheck version, and pinned actionlint workflow lint), and the no-mistakes pre-push gate runs its no-argument full-analysis path; captain instruction retired the separate GitHub Actions Lint job (2026-09-09), so run it locally or through no-mistakes.
...
Local no-mistakes Test is intent-targeted and must not re-run every `tests/*.test.sh`; `.github/workflows/ci.yml` retains only `Repo invariants` as a required PR check (captain instruction, 2026-09-09, retired the broad behavior/lint/coverage GitHub Actions jobs to keep firstmate's automatic CI scoped to its Mac target), and `bin/fm-test-run.sh` remains the owner of the broad behavior suite and its lanes for deliberate local use.
...
bin/fm-lint.sh   # lint that shell surface plus GitHub workflows via pinned actionlint; the no-mistakes gate runs this same single-owner definition
...
bin/fm-test-run.sh --list-lanes   # discover exact lane names, including the portable serial shards (deliberate local use; not automatic CI)
