...
6. Run `no-mistakes` to attach to the pipeline, watch findings, authorize auto-fixes, and review ask-user findings as needed.
   ...
   After the pipeline completes, if it applied a fix or any later commit changes the branch, run `git push no-mistakes` again from the resulting head; a PR synchronize event alone does not refresh the body attestation.
