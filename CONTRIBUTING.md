...
Raising a PR through `no-mistakes` remains the required workflow.
This repo does not register a separate GitHub Actions check that re-verifies the signature and attestation on the forge side; `tests/fm-no-mistakes-required.test.sh` exercises the pinned shared verifier action directly for anyone who wants to reuse it elsewhere.
...
6. Run `no-mistakes` to attach to the pipeline, watch findings, authorize auto-fixes, and review ask-user findings as needed.
   ...
   After any later commit, push `git push no-mistakes` again so the attestation binds to the current PR head.
...
- This repo is a template for running a firstmate orchestrator agent.
  `AGENTS.md` is the agent's main job description and names when to load bundled firstmate skills; `CLAUDE.md` is a real `@AGENTS.md` pointer to it, and `.claude/skills` plus `.grok/skills` are symlinks to `.agents/skills`.
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, `skills/`, and the checked-in Phynd defaults under `config/`.
  The canonical instruction bridges and their regression evidence are maintained in [`docs/verification/instruction-sources.md`](docs/verification/instruction-sources.md).
...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, pinned shellcheck version, and pinned actionlint workflow lint), and the no-mistakes pre-push gate runs its no-argument full-analysis path.
...
`.no-mistakes.yaml` publishes test evidence to the orphan `no-mistakes/evidence` branch, which shares no history with code branches, and pins the gate's lint command to `bin/fm-lint.sh`.
Local no-mistakes Test is intent-targeted and must not re-run every `tests/*.test.sh`; `.github/workflows/ci.yml` runs the `invariants` job on every PR.
The broad behavior suite, lint, and the coverage guard are deliberate local tools rather than an automatic CI gate; run them through the entry points below.
...
```sh
while IFS= read -r script; do /bin/bash -n "$script" || exit; done < <(bin/fm-lint.sh --list-files)   # syntax-check the shell surface fm-lint.sh will cover
bin/fm-lint.sh   # lint that shell surface plus GitHub workflows via pinned actionlint; the no-mistakes gate runs this
...
`.github/workflows/ci.yml` runs the repository-invariant `invariants` job on every PR; the portable parallel shards, the portable serial lane's separate-runner shards, lint, and the coverage guard are deliberate local tools, run through the entry points above rather than as an automatic CI gate.
...
Tests that need the Herdr lab or an explicit opt-in (the live Herdr guards and the live Pi regression) skip themselves and print the tool or environment gate needed to enable them, so the portable suite remains safe on machines without those tools.
