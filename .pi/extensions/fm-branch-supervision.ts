...
// Ambiguity that is not captain-owned is adjudicated inside fleet, not relayed.
// This wakes MAIN to make call itself. Note identifies itself so main cannot
// mistake incoming outcome for its own earlier answer and lose it.
...
// Same outcome reported again means fleet did not act. Route repeat to MAIN
// instead of spending captain attention; store keeps every repeat either way.
  ...
  pi.registerTool?.({
    ...
    renderResult: (result, options, theme, context) => {
      ...
      const output = result.content
        ...
        .join("\n");
      const shellState = context.state as OutcomesToolShellState;
      // Keep each line's ANSI scope independent, matching Pi stock fallback.
