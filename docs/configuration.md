...
Beyond the durable `state/.subsuper-inject-wedged` marker, it attempts a configured Herdr-native active alert that can reach the captain even when every pane is unreadable.
...
The tracked `.no-mistakes.yaml` sets `test.evidence.store_in_repo: true` and pins `commands.lint` to the canonical lint owner, `bin/fm-lint.sh`.
...
Cursor typed-submit confirmation is verified on Herdr only; retained-adapter transcripts are historical, non-operational regression evidence and must not be used for live Firstmate work.
...
`config/crew-dispatch.json` is the tracked Phynd configuration containing natural-language rules that firstmate reads before dispatching a crewmate or scout.
...
Keyed captain answers from built-in adapters use one more seam of the same kind, and runner still decides nothing about them.
Some built-in sources carry captain's answer to a captain-held task, whose meaning belongs to `bin/fm-captain-hold.sh` keyed-answer intake rather than any channel.
A built-in source bound with `bin/fm-captain-hold.sh bind` normally passes each captured result to `bin/fm-procevent-<adapter>.sh answers <result-file>` and pipes output into intake.
Lavish feature-intake registration is explicit exception: runner withholds keyed rows until `bin/fm-lavish-intake.sh record` validates captured payload and performs exact release through captain-hold owner.
