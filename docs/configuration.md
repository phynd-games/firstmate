...
The tracked `.no-mistakes.yaml` sets `test.evidence.store_in_repo: true` and pins `commands.lint` to `bin/fm-lint.sh`, the single owner of the lint definition run locally and by the gate; automatic CI no longer runs a separate lint job.
