...
When a Herdr home cannot prove that its primary harness loaded a continuity owner, `bin/fm-herdr-supervisor.sh` supplies the tracked fallback owner; [`herdr-supervisor.md`](herdr-supervisor.md) owns its eligibility, recovery, and external boundary.
...
Its supervisor injection path supports Herdr panes only in the active runtime, with `FM_SUPERVISOR_BACKEND` and `FM_SUPERVISOR_TARGET` resolved independently from the task-spawn backend.
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`; Herdr uses native agent-state submit confirmation on idle baselines, a composer empty fallback when native stays idle, and a pre-Enter rendered-footer transition when that baseline is unavailable.
The retries-exhausted queued-Enter decision is owned by `fm_composer_queued_enter_verdict` in `bin/fm-composer-lib.sh`; Herdr provides the active backend-specific busy signal.
...
`bin/fm-backend.sh` centralizes backend selection, `state/<id>.meta` helpers, metadata-only cleanup identity validation, selector resolution, and operation dispatch; `bin/backends/herdr.sh` is the one active adapter ([`docs/herdr-backend.md`](herdr-backend.md)).
Herdr is the sole supported runtime backend (`AGENTS.md` hard rule 6), and `bin/fm-backend-policy-lib.sh` is the single owner of that invariant: it names the active backend, the retained legacy adapters, and the one refusal diagnostic, and every boundary consults it - `fm_backend_name` (FM_BACKEND, `config/backend`), `fm_backend_validate` (which every dispatcher reaches through `fm_backend_source`, plus the direct `fm_backend_target_exists` arm), `fm_backend_of_meta` and `fm_backend_validate_task_endpoint` (task records), selector resolution, `bin/fm-supervisor-target-lib.sh` (away-mode pane discovery), and the daemon's supported-set check.
[`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns the operator contract, including the read-only handling of legacy records; `fm_backend_detect` is inert in the active runtime because runtime markers are never a selection input.
...
They survive only as historical files for migration and audit purposes. No Firstmate process can activate them, and active behavior tests exercise Herdr-only refusal or Herdr lab behavior.
The retained-adapter regression lane is armed only by `bin/fm-test-run.sh` in its private test world, which provisions a runner trust artifact alongside the lane variables; exported variables without that artifact are inert, and this boundary prevents accidental activation rather than providing a security boundary against a process that can alter the test world.
...
Retained tmux, zellij, orca, and cmux adapters expose no native busy primitive; their lifecycle behavior is historical and non-operational.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll; Herdr owns the active secondmate recovery classifier, while retained-adapter classifiers remain historical reference only.
Herdr is declared, never detected: Treehouse remains its worktree provider, [`herdr-backend.md`](herdr-backend.md) owns current setup and safety limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#herdr) owns active empirical evidence, including the [Herdr-only invariant](verification/runtime-backends.md#herdr-only-runtime-invariant) record.
...
The retained adapters keep their own pages as historical, non-operational reference: [`zellij-backend.md`](zellij-backend.md) (one shared session, one tab per task), [`orca-backend.md`](orca-backend.md) (Orca owned both worktree and terminal, recording `orca_worktree_id=` and `terminal=`), and [`cmux-backend.md`](cmux-backend.md) (one workspace per task, macOS GUI), each pointing at its retained evidence in [`verification/runtime-backends.md`](verification/runtime-backends.md).
Codex App support is recorded in `docs/codex-app-backend.md`; it is not selectable as a runtime backend.

## Worktrees, not branches in your checkout

Crewmates never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for every Herdr task.
For ship and scout work, `fm-spawn.sh` refuses to launch unless the resolved task path is a real git worktree root that is distinct from the project primary checkout.
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: no worker starts until its clean task worktree matches the exact approved target base when one is recorded, otherwise the fetched tip of origin's resolved default branch, and any unsafe or unverifiable base stops the spawn.
...
Remote placement pins the remote second-mate agent and every worker in its home to Herdr; inherited non-Herdr backend configuration is refused, and every non-doctor primary-to-remote `fm-on` command runs through the remote account's Firstmate-owned job worker rather than its SSH process or a Herdr pane.
...
Worker launch briefs and marked secondmate instructions share the bounded `firstmate.agent-interchange.v1` structural inspection contract, whose digest binds body bytes while explicitly proving neither provenance nor authorization.
...
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` verifies the frozen approved base ref/SHA and, when task meta records `pr=`, always fetches and compares against `refs/pull/<n>/head` by default (recorded `pr_head=` is only an offline fallback) before falling back to the local branch with a warning.
It resolves and prints immutable full base, head, and merge-base SHAs before diffing the exact merge-base and head so a findings-first review can retain exact evidence.
...
Worker replacement after a restored Herdr layout follows the [launch settlement boundary](launch-records.md#what-the-obligation-does).
