...
Herdr is the sole supported runtime backend (`AGENTS.md` hard rule 6), and `bin/fm-backend-policy-lib.sh` is the single owner of that invariant: it names the active backend, the retained legacy adapters, the one refusal diagnostic, and the test-lane predicate, and every boundary consults it - `fm_backend_name` (FM_BACKEND, `config/backend`), `fm_backend_validate` (which every dispatcher reaches through `fm_backend_source`, plus the direct `fm_backend_target_exists` arm), `fm_backend_of_meta` and `fm_backend_validate_task_endpoint` (task records), selector resolution, `bin/fm-supervisor-target-lib.sh` (away-mode pane discovery), and the daemon's supported-set check.
...
They survive only inside the repository-owned regression lane, whose harness identity and root are exported by `tests/lib.sh` and whose caller must be a repository test process, where the pre-invariant contract still holds - tmux default, innermost-first detection, absent `backend=` meaning tmux - so the roughly one hundred fake-tmux suites keep exercising fm-spawn, fm-send, fm-watch, fm-teardown, and the daemon until those fixtures are ported, while `tests/fm-backend-herdr-only.test.sh` strips the lane to prove the real refusals.
...
Retained tmux, zellij, orca, and cmux adapters expose no native busy primitive at all; their lifecycle behavior is relevant only inside the regression lane.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll; Herdr owns the active secondmate recovery classifier, while retained-adapter classifiers remain regression-lane compatibility only.
