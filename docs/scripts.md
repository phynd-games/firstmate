...
| `fm-docs-reader-serve.py` | Reader process entry point that binds its native identity before starting MkDocs |
...
| `fm-record-read.py`      | Read one bounded fleet record through a contained, descriptor-backed handle          |
| `fm_record_io.py`        | Shared descriptor-anchored record reads, report discovery, and freshness fingerprints |
...
| `fm-test-run.sh`         | Behavior-test runner: selection, portable lanes, proven-isolated bounded concurrency, budgets, coverage guard, timing/JSON |
...
| `fm-test-isolation-proof.sh` | Concurrent isolation proof and portable candidate set owner |
...
| `fm-review-diff.sh`      | Review exact base/head SHAs for a crewmate branch or resolved PR head                 |
...
| `fm-herdr-supervisor.sh` | Keep watcher continuity alive from a Herdr-tracked pane when the primary harness has no owner |
| `fm-crew-state.sh`       | Print one deterministic current-state line for a crew                                |
| `fm-nm-run-lib.sh`       | Single owner of shared no-mistakes branch-and-code run-attribution primitives and rules |
| `fm-validation-loop-lib.sh` | Deterministic limits bounding automatic validation-loop continuation, journal folding, stop verdicts, and thresholds |
...
| `fm-branch-outcome.sh`   | Own the supervision branch's append-only outcome store, read cursor, session-start replay, and routine-note duplicate coalescing gate |
