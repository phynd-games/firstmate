...
`bin/fm-procevent.sh` is the captured-result channel: after capture, an ordinary bound source has its result passed to `bin/fm-procevent-<adapter>.sh answers <result-file>` and whatever that prints is piped into the intake, so any adapter with an `answers` command works and the runner names no adapter, parses no result, and carries no decision rule.
Lavish intake is the exception: its keyed rows remain unhandled until `bin/fm-lavish-intake.sh record` validates the exact captured payload and routes one exact release through captain-hold.
