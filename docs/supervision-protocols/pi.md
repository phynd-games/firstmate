...
6. Ordinary same-process session replacement (`/new`, `/resume`, `/fork`, reload) retires only the prior generation; the replacement `session_start` automatically begins the new cycle without restarting Pi.
...
9. An unexpected child close enters bounded exponential retry, and an exhausted retry or lost session lock is surfaced as a watcher failure instead of disappearing.
   After retry exhaustion, the extension relinquishes its loaded-owner marker so the Herdr fallback can take over the same home.
