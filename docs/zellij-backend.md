...
Herdr is Firstmate's sole supported runtime backend (`AGENTS.md` hard rule 6); selecting `zellij` through `config/backend`, `FM_BACKEND`, or `--backend` is refused by name, and `bin/backends/zellij.sh` stays on disk only as historical, non-operational reference.
