...
The reader is MkDocs, unmodified, running its own development server with its own navigation, search, theme, and live reload.
Firstmate contributes a generated private configuration and one hook module, `bin/fm-docs-reader-hooks.py`, that narrows the site and sanitizes each page.
