...
# Probe an arbitrary same-origin path so readiness can prove the APPLICATION is
# served, not merely that the process answers. Proxies are disabled explicitly:
# a loopback check must never leave the machine.
probe_path() {  # <port> <path> -> prints the HTTP status
  if command -v curl >/dev/null 2>&1; then
    curl -fsS -o /dev/null -w '%{http_code}' --noproxy '*' --max-time 3 \
      "http://127.0.0.1:$1$2" 2>/dev/null || printf '000'
    return 0
  fi
  python3 - "$1" "$2" <<'PYPROBE' 2>/dev/null || printf '000'
import sys, urllib.request
opener = urllib.request.build_opener(urllib.request.ProxyHandler({}))
try:
    with opener.open("http://127.0.0.1:%s%s" % (sys.argv[1], sys.argv[2]), timeout=3) as r:
...
record_load() {
  [ "$OWNER_RECORD_LOADED" -eq 1 ] && [ -z "$OWNER_RECORD_ERROR" ] && return 0
  [ "$OWNER_RECORD_LOADED" -eq 1 ] && [ -n "$OWNER_RECORD_ERROR" ] && return 1
  OWNER_RECORD_LOADED=1
  OWNER_RECORD_TEXT=''
  OWNER_RECORD_ERROR=''
  if ! OWNER_RECORD_TEXT=$(python3 "$SCRIPT_DIR/fm-dashboard-read.py" \
      "$RECORD" "$STATE" text "$OWNER_RECORD_MAX_BYTES" - /dev/null 2>&1); then
    OWNER_RECORD_ERROR=$(printf '%s' "$OWNER_RECORD_TEXT" | tail -1)
    OWNER_RECORD_TEXT=''
    [ -n "$OWNER_RECORD_ERROR" ] || OWNER_RECORD_ERROR='could not read the owner record'
    return 1
  fi
  return 0
}
...
record_get() {  # <key>
  record_load || return 1
  printf '%s\n' "$OWNER_RECORD_TEXT" | sed -n "s/^$1=//p" | head -1
}

record_fingerprint() {
  record_load || return 1
  digest_of "$OWNER_RECORD_TEXT"
}
...
record_drop() { rm -f -- "$RECORD"; record_cache_reset; }

startup_journal_record_load() {
  [ "$JOURNAL_LOADED" -eq 1 ] && [ -z "$JOURNAL_ERROR" ] && return 0
  [ "$JOURNAL_LOADED" -eq 1 ] && [ -n "$JOURNAL_ERROR" ] && return 1
  JOURNAL_LOADED=1
  JOURNAL_TEXT=''
  JOURNAL_ERROR=''
  if ! JOURNAL_TEXT=$(python3 "$SCRIPT_DIR/fm-dashboard-read.py" \
      "$JOURNAL" "$STATE" text "$JOURNAL_MAX_BYTES" - /dev/null 2>&1); then
    JOURNAL_ERROR=$(printf '%s' "$JOURNAL_TEXT" | tail -1)
    JOURNAL_TEXT=''
    [ -n "$JOURNAL_ERROR" ] || JOURNAL_ERROR='could not read the startup journal'
    return 1
  fi
  return 0
}

startup_journal_cache_reset() {
  JOURNAL_LOADED=0
  JOURNAL_TEXT=''
  JOURNAL_ERROR=''
}
...
startup_journal_get() {  # <key>
  startup_journal_record_load || return 1
  printf '%s\n' "$JOURNAL_TEXT" \
...
startup_journal_load() {
  startup_journal_record_load || return 0
...
startup_journal_drop() { rm -f -- "$JOURNAL"; startup_journal_cache_reset; }
...
await_ready() {  # <port> <digest> <session> <workspace> <tab> <pane>
  ...
  while [ "$tries" -gt 0 ]; do
    if health=$(probe_health "$port") && health_is_ours "$health" "$digest" \
      ...
      # Health alone only proves the process answers. The promise is a usable
      # dashboard, so the application shell and its bundle must answer too
      # before this reports ready.
      if [ "$(probe_path "$port" /)" = 200 ] \
        && [ "$(probe_path "$port" /assets/app.js)" = 200 ]; then
        return 0
      fi
      FM_DASHBOARD_ASSETS_UNSERVED=1
    fi
...
command_ensure() {
  ...
  # Refuse before touching Herdr: a missing client would otherwise start a pane
  # that can only ever serve an error page.
  if ! "$SCRIPT_DIR/fm-dashboard-build.sh" verify >/dev/null 2>&1; then
    blocked "the dashboard client is not built; run bin/fm-dashboard-build.sh build"
    return 1
  fi
  ...
  if [ "$ready_rc" -ne 0 ]; then
    ...
    dashboard_lock_release >/dev/null 2>&1
    if [ -n "${FM_DASHBOARD_ASSETS_UNSERVED:-}" ]; then
      blocked "the dashboard answered health on port $port but never served its application assets"
    else
...
command_status_report() {
  local before after
  if ! record_load; then
    if [ "$OWNER_RECORD_ERROR" = "not present" ]; then
      printf 'dashboard: no owner recorded for this home\n'
      return 0
    fi
    printf 'DASHBOARD_BLOCKED: the dashboard owner record could not be read: %s\n' \
      "$OWNER_RECORD_ERROR"
    return 1
  fi
  printf 'dashboard: recorded owner\n'
  printf '%s\n' "$OWNER_RECORD_TEXT" \
    | sed -nE 's/^(schema|home|session|workspace|tab|pane|port|url|started)=/  \1: /p'
