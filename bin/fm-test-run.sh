...
family_for_basename() {
  case "$1" in
    ...
    fm-operational-input.test.sh|fm-pi-primary-types.test.sh|fm-pi-startup-profile.test.sh|\
    ...
    fm-watch-triage.test.sh|fm-task-inbox.test.sh|fm-handoff-confirm.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-factory-manifest.py|bin/firstmate_factory/*|\
    assets/local-software-factory-m1/*)
      # Deliberately the WHOLE family. This runner executes every pure-contract
      # unit script, while factory owners also require that complete family.
      printf '%s\n' pure-contract-unit
      ;;
    ...
    bin/fm-peek.sh|bin/fm-composer*)
      printf '%s\n' backend-dispatch
      printf '%s\n' pure-contract-unit
      # fm-send registers the handoff obligation as it writes the record, so a
      # change here also selects the confirmed-handoff family.
      printf '%s\n' watcher-wake-lock
      ;;
