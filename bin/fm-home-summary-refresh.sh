...
if [ -n "${FM_HOME:-}" ]; then
  FM_HOME=$FM_HOME
elif [ -n "${FM_STATE_OVERRIDE:-}" ] && [ -n "${FM_DATA_OVERRIDE:-}" ] && [ -n "${FM_CONFIG_OVERRIDE:-}" ] \
  && [ "$(dirname "$FM_STATE_OVERRIDE")" = "$(dirname "$FM_DATA_OVERRIDE")" ] \
  && [ "$(dirname "$FM_STATE_OVERRIDE")" = "$(dirname "$FM_CONFIG_OVERRIDE")" ]; then
  # Synthetic and test homes pass all private roots explicitly while keeping
  # FM_ROOT_OVERRIDE on tracked code. Derive containment root only when those
  # private roots agree on one parent.
  FM_HOME=$(dirname "$FM_STATE_OVERRIDE")
else
