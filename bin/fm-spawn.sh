  ...
  herdr)
    ...
    if [ "$KIND" != secondmate ] && fm_backend_herdr_presentation_enabled "$CONFIG" "$STATE"; then
      ...
      if [ -e "$HERDR_PRESENTATION_JOURNAL" ] || [ -L "$HERDR_PRESENTATION_JOURNAL" ]; then
        ...
        # Recovery holds the lock through the existing launch handoff. That
        # handoff can legitimately exceed the fresh-projection fallback window
        # while Treehouse settles a worktree, so wait longer but remain bounded.
