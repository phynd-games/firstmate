...
When it is implemented and committed, push your branch, then open a PR through \`$FM_ROOT/bin/fm-pr-create.sh $ID\` with the arguments intended for \`gh-axi pr create\`; it refuses unless the durable self-review is valid, then forwards those arguments. Append \`done: PR {url}\` to the status file and stop.
...
The task is complete only when committed on your branch.
Before reporting completion, run \`$FM_ROOT/bin/fm-pr-self-review-check.sh $ID no-mistakes\`; stop if it refuses.
