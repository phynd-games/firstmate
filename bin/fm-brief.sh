...
- Approving a review step while findings are still open is firstmate's disposition, not yours: it means continue the pipeline despite them, so firstmate must first classify each one and decide what is fixed and what is dismissed. Report the open findings and apply the disposition it returns.
