...
_fm_vloop_findings_valid() {  # <evidence-content>
  printf '%s\n' "$1" | awk '
    ...
    function valid_scalar(value) {
      ...
      # gate), "<N> awaiting, <N> auto-fix" (a run parked at a gate while
      # separate auto-fixes are also in flight), or a severity breakdown with
      # only the present severities (each count >= 1) joined by ", " in fixed
      # descending-severity order error, warning, info. The captured
      # compatibility sample lives in tests/fm-validation-loop.test.sh;
      # unfamiliar shapes fail closed.
      ...
      return value == "none" \
        ...
        || value ~ /^[1-9][0-9]* awaiting, [1-9][0-9]* auto-fix$/ \
        || value ~ /^[1-9][0-9]* info$/ \
