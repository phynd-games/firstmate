...
run_spawn() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" "$proj" --scout \
    --not-applicable "configuration: task=$id; target=tests/fm-tangle-guard.test.sh tangle guard fixture; action=exercise test behavior without product change" >/dev/null
