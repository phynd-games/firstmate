...
#
# bin/fm-lint.sh must be the single owner that the pre-push gate
# (.no-mistakes.yaml commands.lint) invokes. A captain instruction
# (2026-09-09) retired the GitHub Actions Lint job that used to invoke it in
# CI (.github/workflows/ci.yml), so the no-mistakes gate is now the sole
# automatic enforcer; this suite still pins the same definition for local use.
...
# validation yet failed that exact check in the CI Lint job that existed then,
# on info/warning findings such as SC2015, SC1007, and SC2034. A second axis
# was tool-version skew: that job's ShellCheck floated with the runner image
# and still emitted SC2015, which ShellCheck retired in 0.11.0. fm-lint.sh now
# pins one exact version so command, file set, config, AND version all match
# wherever it runs.
set -u
