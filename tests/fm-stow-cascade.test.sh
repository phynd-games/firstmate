...
run_cascade() { # <primary-home> [env assignments...]
  ...
  env -i \
    ...
    HOME="${HOME:-/tmp}" \
    TMPDIR="${TMPDIR:-/tmp}" \
    FM_BACKEND_LEGACY_TEST_LANE="${FM_BACKEND_LEGACY_TEST_LANE:-}" \
    FM_BACKEND_TEST_HARNESS="${FM_BACKEND_TEST_HARNESS:-}" \
    FM_BACKEND_TEST_RUNNER_ROOT="${FM_BACKEND_TEST_RUNNER_ROOT:-}" \
    FM_BACKEND_TEST_TRUST_FILE="${FM_BACKEND_TEST_TRUST_FILE:-}" \
