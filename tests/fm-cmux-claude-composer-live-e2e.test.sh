...
FM_HOME="$LAB" "$ROOT/bin/fm-brief.sh" "$TASK" comms --scout \
  --not-applicable "configuration: target=tests/fm-cmux-claude-composer-live-e2e.test.sh live cmux composer probe; action=exercise composer behavior without product implementation" \
