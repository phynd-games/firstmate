...
test_session_start_omits_removed_dashboard_and_preserves_supervision() {
  local rec root home fakebin out
  rec=$(new_world dashboard-removed)
  IFS='|' read -r root home fakebin <<EOF
$rec
EOF
  make_fake_toolchain "$fakebin"
  ...
  assert_not_contains "$out" "DASHBOARD" \
    "session start still emitted the removed dashboard section"
  assert_not_contains "$out" "FIRSTMATE_DASHBOARD_URL" \
    "session start still emitted a removed dashboard URL"
  assert_contains "$out" "SUPERVISION OPERATING INSTRUCTIONS" \
    "session start lost its supervision instructions"
  assert_contains "$out" "NEXT STEP" \
    "session start did not complete after dashboard removal"
