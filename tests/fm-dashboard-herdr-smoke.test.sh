...
test_a_real_herdr_pane_serves_the_dashboard_and_reports_its_url() {
  ...
  curl -fsS --max-time 10 "$url" | grep -q 'id="root"' \
    || fail "the served page does not carry the React mount"
  curl -fsS --max-time 10 "$url" | grep -q 'src="/assets/app.js"' \
    || fail "the served page does not reference the bundled React client"
