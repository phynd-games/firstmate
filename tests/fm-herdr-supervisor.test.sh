...
out=$(run_supervisor "$HOME10" "$FAKEBIN" ensure 2>&1) && fail "a replaced server permitted duplicate supervision"
assert_present "$HOME10/state/.herdr-supervisor" "the unresolved prior binding remains authoritative"
...
pass "server replacement retains unresolved ownership and blocks establishment"
stop_loop "$HOME10"
