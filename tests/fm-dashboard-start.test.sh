...
test_diagnostics_log_trimming_reads_only_a_bounded_tail() {
  ...
  for i in $(seq 1 5000); do
    printf 'old-%s\n' "$i" >> "$home/state/.dashboard-start.log"
  done
  port=$(free_port)
  out=$(start "$home" "$port" ensure) || fail "ensure failed with a large diagnostics log: $out"
  lines=$(wc -l < "$home/state/.dashboard-start.log" | tr -d '[:space:]')
  [ "$lines" -le 200 ] || fail "diagnostics log exceeded its line bound: $lines"
  pass "startup diagnostics retain only a bounded tail"
}
...
SH
  ...
  code=$(curl -sS --max-time 6 -o "$home/timeout-response" \
    -w '%{http_code}' "http://127.0.0.1:$port/api/v1/overview" 2>/dev/null || true)
  [ "$code" = "503" ] \
    || fail "a timed-out evidence collection returned HTTP $code instead of 503"
  child=
...
SH
  ...
  code=$(curl -sS --max-time 6 -o "$home/timeout-response" \
    -w '%{http_code}' "http://127.0.0.1:$port/api/v1/overview" 2>/dev/null || true)
  [ "$code" = 503 ] || fail "a contained timeout returned HTTP $code instead of 503"
  child=
  for _ in $(seq 1 20); do
    if [ -s "$home/descendant.pid" ]; then
...
# The old server proved cleanup by enumerating descendants with `ps`, so a
# broken `ps` made cleanup unprovable and health failed closed. The collector
# now runs in its OWN PROCESS GROUP and is reclaimed with killpg, which does not
# consult `ps` at all, so that failure mode no longer exists. What replaces it is
# a stronger claim, asserted here: the descendant is actually gone.
test_a_timed_out_collection_reclaims_its_whole_process_group() {
  local home port out child code health alive
  home=$(make_home reclaim-group)
  port=$(free_port)
  out=$(PATH="$home/fakebin:$PATH" FM_HOME="$home" FAKE_HERDR_STATE="$home/herdr" \
    FM_DASHBOARD_BUILD_TIMEOUT=2 FM_DASHBOARD_DESCENDANT_MARKER="$home/descendant.pid" \
    FM_DASHBOARD_PORT="$port" "$START" ensure 2>&1) \
    || fail "ensure failed: $out"
  # A collector that hangs AND leaves a grandchild behind.
  cat > "$home/fakebin/jq" <<'SH'
#!/usr/bin/env bash
set -u
marker="${FM_DASHBOARD_DESCENDANT_MARKER:?}"
...
SH
  ...
  code=$(curl -sS --noproxy '*' --max-time 20 -o /dev/null \
    -w '%{http_code}' "http://127.0.0.1:$port/api/v1/overview" 2>/dev/null || true)
  [ "$code" = "503" ] \
    || fail "a timed-out collection returned HTTP $code instead of 503"

  child=
  for _ in $(seq 1 30); do
    if [ -s "$home/descendant.pid" ]; then child=$(cat "$home/descendant.pid"); break; fi
    sleep 0.2
  done
  [ -n "$child" ] || fail "the fixture did not create its descendant marker"

  alive=1
  for _ in $(seq 1 30); do
    if ! kill -0 "$child" 2>/dev/null; then alive=0; break; fi
    sleep 0.2
  done
  if [ "$alive" = 1 ]; then
    kill -KILL "$child" 2>/dev/null || true
    fail "a timed-out collection left its descendant $child running"
  fi

  # Cleanup was proven, so health must NOT be degraded. Health degrades only
  # when the group cannot be proven reclaimed.
  health=$(curl -sS --noproxy '*' --max-time 3 "http://127.0.0.1:$port/healthz" 2>/dev/null || true)
  printf '%s' "$health" | jq -e '.ready == true' >/dev/null \
    || fail "health was degraded even though the group was reclaimed: $health"
  pass "a timed-out collection returns 503 and reclaims its whole process group"
}
