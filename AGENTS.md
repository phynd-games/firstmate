...
6. **Network checks** - after the fleet-state digest, the deferred stage's result, or an explicit statement of what it has not confirmed yet.
   ...
   The `DASHBOARD` section that follows brings up this home's local read-only dashboard and prints its proven `FIRSTMATE_DASHBOARD_URL=`; it is local-only, never fails session start, and prints `DASHBOARD_BLOCKED:` instead of a URL whenever readiness could not be proved (`bin/fm-dashboard-start.sh`).
